...
// TEMPORARY — devtest only. Seeds InventoryLot + InventoryTransaction
// records for a TEST_ product directly (bypassing the whole PO/receiving
// flow) so Go Live's multi-lot Record Sale / cost logic can be verified
// against real lot data. Removed before merging this branch's PR.
export async function POST(req: Request) {
  const body = await req.json().catch(() => null);
  const productId = typeof body?.productId === "string" ? body.productId : "";
  const lots = Array.isArray(body?.lots) ? body.lots : [];
  if (!productId || !productId.includes("TEST_")) {
    return NextResponse.json({ error: "productId must reference a TEST_ product." }, { status: 400 });
  }
