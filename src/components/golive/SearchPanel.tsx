...
export function SearchPanel() {
  const { selectProduct, addToQueue } = useLiveSession();
  ...
  return (
    <div className="glass-panel rounded-2xl p-5">
      <h3 className="mb-3 font-display text-sm font-bold uppercase tracking-widest text-ld-muted">Search Product</h3>
      <input
        value={query}
      ...
      {results.length > 0 && (
        <div className="mt-3 space-y-2">
          {results.map((p) => (
            <div key={p.id} className="flex items-center gap-3 rounded-xl border border-ld-border bg-ld-bg-elevated p-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-ld-muted">
                  {p.brand} · {p.inventory} in stock · {p.sku}
                </p>
              </div>
      ...
      {query && results.length === 0 && <p className="mt-3 text-sm text-ld-muted">No products match &ldquo;{query}&rdquo;.</p>}
    </div>
  );
}
